...
struct Command {
	...
	void operator()(int& pos, std::unordered_map<char, int>& reg, std::vector<Command>& prog, std::ostringstream& os) {
...
struct Task
{
	...
	static void run() {
		...
		std::ifstream inputStream("2016/25.txt");
		...
		while (inputStream >> line)
		{
			...
			if (line == "inc" || line == "dec" || line == "tgl" || line == "out") {
				char address;
				inputStream >> address;
		...
		do {
			...
			while (pos >= 0 && pos < program.size()) {
				bool shouldCheckOut = program[pos].name == "out";
				program[pos](pos, reg, program, outStream);
				if (shouldCheckOut) {
					if (outStream.str().size() > 1) {
						if (outStream.str()[outStream.str().size() - 1] == outStream.str()[outStream.str().size() - 2]) {
							break;
						}
						checkCnt++;
						if (checkCnt >= thr) {
